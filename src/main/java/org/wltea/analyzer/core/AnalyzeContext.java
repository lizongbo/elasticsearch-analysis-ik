...
class AnalyzeContext {
    ...
    void initCursor() {
        this.cursor = 0;
        this.segmentBuff[this.cursor] = CharacterUtil.regularize(this.segmentBuff[this.cursor], cfg.isEnableLowercase(), cfg.isEnableCht2Chs());
     ...
     * <p>
     * 满足一下条件时， 1.available == BUFF_SIZE 表示buffer满载 2.buffIndex < available - 1 && buffIndex > available - BUFF_EXHAUST_CRITICAL表示当前指针处于临界区内
     * 3.!context.isBufferLocked()表示没有segmenter在占用buffer 要中断当前循环（buffer要进行移位，并再读取数据的操作）
     *
     * @return
     */
    boolean needRefillBuffer() {
        return this.available == BUFF_SIZE && this.cursor < this.available - 1 && this.cursor > this.available - BUFF_EXHAUST_CRITICAL
            && !this.isBufferLocked();
    }
                ...
                while (l != null) {
                    ...
                    if (l != null) {
                        ...
                        for (; index < l.getBegin(); index++) {
                            this.outputSingleCJK(index, 1);
                        }
                    }
                }
            } else { //pathMap中找不到index对应的LexemePath
                char c = this.segmentBuff[index];
                //单字输出
                int codePoint = Character.codePointAt(this.segmentBuff, index);
                if (isGB18030ExtCodePoint(codePoint)) {
                    this.outputSingleCJK(index, 2);
                    index = index + 2;
                } else {
                    this.outputSingleCJK(index, 1);
                    index++;
    ...
    /**
     * 判断指定的代码点是不是GB18030-2022及修改单所允许的生僻字字符
     *
     * @param codePoint 大于65536的代码点
     * @return
     */
    public boolean isGB18030ExtCodePoint(int codePoint) {
        if (codePoint >= 0x20000 && codePoint <= 0x2A6DF) { //CJK扩展区B
            return true;
        }
        if (codePoint >= 0x2A700 && codePoint <= 0x2B739) { //CJK扩展区C
            return true;
        }
        if (codePoint >= 0x2B740 && codePoint <= 0x2B81D) { //CJK扩展区D
            return true;
        }
        if (codePoint >= 0x2B820 && codePoint <= 0x2CEA1) { //CJK扩展区E
            return true;
        }
        if (codePoint >= 0x2CEB0 && codePoint <= 0x2EBE0) { //CJK扩展区F
            return true;
        }
        if (codePoint >= 0x30000 && codePoint <= 0x3134A) { //CJK扩展区G
            return true;
        }
        if (codePoint >= 0x31350 && codePoint <= 0x323AF) { //CJK扩展区H
            return true;
        }
        if (codePoint >= 0x2EBF0 && codePoint <= 0x2EE4F) { //CJK扩展区I
            return true;
        }
        return false;
    }
     ...
     * @param length 1或者2
     */
    private void outputSingleCJK(int index, int length) {
        if (CharacterUtil.CHAR_CHINESE == this.charTypes[index]) {
            Lexeme singleCharLexeme = new Lexeme(this.buffOffset, index, length, Lexeme.TYPE_CNCHAR);
            this.results.add(singleCharLexeme);
        } else if (CharacterUtil.CHAR_OTHER_CJK == this.charTypes[index]) {
            Lexeme singleCharLexeme = new Lexeme(this.buffOffset, index, length, Lexeme.TYPE_OTHER_CJK);
            this.results.add(singleCharLexeme);
        }
    }
